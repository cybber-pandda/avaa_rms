...
const IcoBuilding = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 21V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2v13" />
        <path d="M7 21v-9a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v9" />
    </svg>
);
...
function BlockEmployerForm({ onBlock }: { onBlock: (user: SearchResult & { reason?: string }) => void }) {
    ...
    const handleBlock = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to block employer:', response.status);
            }
        } catch (error) {
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Block an Employer</h3>
            ...
            <div className="relative mb-4">
                ...
                <input
                    ...
                    placeholder="Search employers by email..."
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500">{selectedUser.company_name}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Why are you blocking this employer?"
                            rows={3}
                    ...
                    >
                        <IcoBlock />
                        <span>{isBlocking ? 'Blocking...' : 'Block Employer'}</span>
                    </button>
                </div>
            )}
        </div>
...
/* ── Main Component ── */
export default function JobSeekerBlockedUsers({ auth }: PageProps<{ auth: any }>) {
    ...
    return (
        <AppLayout>
            <Head title="Blocked Employers - Job Seeker Settings" />
            
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
                {/* Header */}
                <div>
                    <h2 className="text-xl font-bold text-gray-900">Blocked Employers</h2>
                    <p className="text-sm text-gray-600 mt-1">
                        Manage employers you've blocked from contacting you.
                    </p>
                </div>

                {/* Block New Employer */}
                <BlockEmployerForm onBlock={handleBlock} />

                {/* Blocked Employers List */}
                <div>
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">
                        Blocked Employers ({blockedUsersList.length})
                    </h3>
                    
                    {blockedUsersList.length === 0 ? (
                        <div className="text-center py-8 bg-white border border-gray-200 rounded-xl">
                            <IcoBuilding />
                            <p className="text-sm text-gray-600 mt-2">No blocked employers</p>
                            <p className="text-xs text-gray-400 mt-1">
                                Employers you block will appear here
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-3">
